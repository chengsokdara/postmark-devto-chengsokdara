...
export const extractJobApplication = async (
  emailText: string,
  resumeText?: string,
): Promise<ExtractedJobApplication> => {
  try {
    ...
    const extracted = responseContent as ExtractedJobApplication;
...
export const validateStructure = (
  result: unknown,
): result is ExtractedJobApplication => {
  try {
    extractedJobApplicationSchema.parse(result);
    return true;
  } catch {
    return false;
  }
...
export const validateSemantics = (result: ExtractedJobApplication): boolean => {
  const emailOk =
